                          ...
                          void (*finish)())
{
    ...
    self->plat = wgbm_platform(dpy->wegl.wcore.platform);
    self->drm_fd = -1;
    self->dmabuf_fd = -1;
    self->finish = finish;
...
drm_display_create(int fd, struct wgbm_platform *plat)
{
    ...
    // This dlopen() works around a linkage problem in older versions of mesa.
    // The workaround was added to mesa by commit 828f1333 so we might want
    // to remove it from here eventually.
...
wnull_display_destroy(struct wcore_display *wc_self)
{
    ...
    if (self->drm)
        drm_display_destroy(self->drm, wgbm_platform(wc_self->platform));
